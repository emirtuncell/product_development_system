                        ...
                        <v-tooltip bottom>
                            <template v-slot:activator="{ on, attrs }">
                                <v-icon v-bind="attrs" v-on="on" class="mr-2"
                                    @click="$router.push('/mold-product?mold_id=' + item.id)">
                                    mdi-consolidate
                                </v-icon>
                            </template>
                            <span>Ürün İlişkilendir</span>
                        </v-tooltip>

                        <v-tooltip bottom>
                            <template v-slot:activator="{ on, attrs }">
                                <v-icon v-bind="attrs" v-on="on" class="mr-2"
                                    @click="$router.push('/machine-mold?mold_id=' + item.id)">
                                    mdi-tools
                                </v-icon>
                            </template>
                            <span>Makine Ata</span>
                        </v-tooltip>
    ...
    methods: {
        ...
        save() {
            if (this.selected_item.id) {
                this.$api.put("/molds/" + this.selected_item.id, this.selected_item).then((r) => {
                    this.fetchItems()
                    this.dialog = false
                }).catch((e) => {
                ...
                this.$api.post("/molds", this.selected_item).then((r) => {
                    this.fetchItems()
                    this.dialog = false
                }).catch((e) => {
                    console.log(e.response)
        ...
        delete_item() {
            this.$api.delete("/molds/" + this.selected_item.id).then((r) => {
