                        ...
                        <v-tooltip bottom>
                            <template v-slot:activator="{ on, attrs }">
                                <v-icon v-bind="attrs" v-on="on" class="mr-2"
                                    @click="$router.push('/factory-admin?factory_id='+item.id)">
                                    mdi-account
                                </v-icon>
                            </template>
                            <span>Fabrika Admin Kullanıcıları</span>
                        </v-tooltip>
    ...
    methods: {
        ...
        save() {
            if (this.selected_item.id) {
                this.$api.put("/factories/" + this.selected_item.id, this.selected_item).then((r) => {
                    this.fetchItems()
                    this.dialog = false
                }).catch((e) => {
                ...
                this.$api.post("/factories", this.selected_item).then((r) => {
                    this.fetchItems()
                    this.dialog = false
                }).catch((e) => {
                    console.log(e.response)
        ...
        delete_item() {
            this.$api.delete("/factories/" + this.selected_item.id).then((r) => {
