                        ...
                        <v-tooltip bottom>
                            <template v-slot:activator="{ on, attrs }">
                                <v-icon v-bind="attrs" v-on="on" class="mr-2"
                                    @click="$router.push('/machine-production?machine_id=' + item.id).catch(() => { })">
                                    mdi-monitor-screenshot
                                </v-icon>
                            </template>
                            <span>Üretim Ekranı</span>
                        </v-tooltip>

                        <v-tooltip bottom>
                            <template v-slot:activator="{ on, attrs }">
                                <v-icon v-bind="attrs" v-on="on" class="mr-2"
                                    @click="$router.push('/machine-operator?machine_id=' + item.id).catch(() => { })">
                                    mdi-account-key
                                </v-icon>
                            </template>
                            <span>Operatör Ata</span>
                        </v-tooltip>

                        <v-tooltip bottom>
                            <template v-slot:activator="{ on, attrs }">
                                <v-icon v-bind="attrs" v-on="on" class="mr-2"
                                    @click="$router.push('/machine-mold?machine_id=' + item.id).catch(() => { })">
                                    mdi-tools
                                </v-icon>
                            </template>
                            <span>Kalıp Ata</span>
    ...
    methods: {
        ...
        save() {
            if (this.selected_item.id) {
                this.$api.put("/machines/" + this.selected_item.id, this.selected_item).then((r) => {
                    this.fetchItems()
                    this.dialog = false
                }).catch((e) => {
                    console.log(e.response)
                    this.$message.text = e.response.data.detail
                    this.$message.color = "error"
                    this.$message.show = true
                })
            }
            else {
                this.$api.post("/machines", this.selected_item).then((r) => {
                    this.fetchItems()
                    this.dialog = false
                }).catch((e) => {
                    console.log(e.response)
        ...
        delete_item() {
            this.$api.delete("/machines/" + this.selected_item.id).then((r) => {
