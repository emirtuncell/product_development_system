        ...
        <v-dialog v-model="dialog" width="400">
            <v-card>
                ...
                <v-card-text>
                    ...
                    <v-text-field v-model="selected_item.username" 
                        label="Kullanıcı Adı"></v-text-field>
                    <v-text-field v-model="selected_item.password" type="password"
...
export default {
    ...
    methods: {
        ...
        save() {
            this.selected_item.factory_id=this.$route.query.factory_id;
            if (this.selected_item.id) {
                this.$api.put("/factory-admins/" + this.selected_item.id, this.selected_item).then((r) => {
                    this.fetchItems()
                    this.dialog = false
                }).catch((e) => {
                ...
                this.$api.post("/factory-admins", this.selected_item).then((r) => {
                    this.fetchItems()
                    this.dialog = false
                }).catch((e) => {
                    console.log(e.response)
        ...
        delete_item() {
            this.$api.delete("/factory-admins/" + this.selected_item.id).then((r) => {
        ...
        fetchItems() {
            ...
            this.$api.get("/factory-admins/" + this.$route.query.factory_id).then((r) => {
