        ...
        <v-dialog v-model="dialog" width="400">
            <v-card>
                ...
                <v-card-text>
                    ...
                    <v-switch v-model="selected_item.is_planned" label="Planlı"></v-switch>
                </v-card-text>
                <v-card-actions>
                    <v-spacer></v-spacer>
    ...
    methods: {
        ...
        save() {
            if (this.selected_item.id) {
                this.$api.put("/stop-causes/" + this.selected_item.id, this.selected_item).then((r) => {
                    this.fetchItems()
                    this.dialog = false
                }).catch((e) => {
                ...
                this.$api.post("/stop-causes", this.selected_item).then((r) => {
                    this.fetchItems()
                    this.dialog = false
                }).catch((e) => {
                    console.log(e.response)
        ...
        delete_item() {
            this.$api.delete("/stop-causes/" + this.selected_item.id).then((r) => {
