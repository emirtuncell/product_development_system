                        ...
                        <v-tooltip bottom>
                            <template v-slot:activator="{ on, attrs }">
                                <v-icon v-bind="attrs" v-on="on" class="mr-2"
                                    @click="$router.push('/mold-product?product_id=' + item.id)">
                                    mdi-blur-linear
                                </v-icon>
                            </template>
                            <span>Kalıpları Görüntüle</span>
                        </v-tooltip>
    ...
    methods: {
        ...
        save() {
            if (this.selected_item.id) {
                this.$api.put("/products/" + this.selected_item.id, this.selected_item).then((r) => {
                    this.fetchItems()
                    this.dialog = false
                }).catch((e) => {
                ...
                this.$api.post("/products", this.selected_item).then((r) => {
                    this.fetchItems()
                    this.dialog = false
                }).catch((e) => {
                    console.log(e.response)
        ...
        delete_item() {
            this.$api.delete("/products/" + this.selected_item.id).then((r) => {
